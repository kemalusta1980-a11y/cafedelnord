...
- İterasyon 18 (redesign): Kullanıcının referans görseline göre görsel yeniden tasarım — vurgu rengi altından KIRMIZIYA (#e63946, --gold değişkeni üzerinden; class adları 'gold' kaldı), büyük başlıklar (h1/h2.font-display) Cormorant Garamond serif, hero split layout (sol metin+CTA, sağ dairesel kumpir görseli, hero-dish), btn-solid kırmızı+beyaz metin, kart fiyat rozeti tamamen kaldırıldı (fiyat/sepet/sipariş yasak — kullanıcı kuralı). İçerik/route/özellikler değişmedi. Testler: desktop+mobil tüm sayfalar, yatay taşma yok, %100 PASS (iteration_12.json). NOT: Yayın için yeniden deploy gerekli. — zararsız ResizeObserver loop hataları craco devServer overlay filtresi + index.js global error listener ile bastırıldı. Testing agent: tüm sayfalar + resize/scroll stres testleri PASS, hiçbir overlay/hata yok (iteration_3.json)
