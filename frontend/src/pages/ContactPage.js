      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8">
        <Reveal>
          ...
          <h1 className="font-display font-black text-4xl sm:text-5xl lg:text-6xl tracking-tighter mb-12">
            İleti<span className="text-gold">şim</span>
          </h1>
        </Reveal>
            ...
            <div className="space-y-5">
              <div className="card-dark p-6 flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="font-display font-bold text-lg mb-1">Telefon</h3>
                  {s.phone ? (
                    ...
                    <p className="text-white/30 italic text-sm">Admin panelinden eklenebilir</p>
                  )}
                </div>
              </div>
              ...
              <div className="card-dark p-6 flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="font-display font-bold text-lg mb-1">Adres</h3>
                  {s.address ? (
                    <p className="text-white/60" data-testid="contact-address">{s.address}</p>
                  ) : (
                    <p className="text-white/30 italic text-sm" data-testid="contact-address-missing">Adres bilgisi admin panelinden eklenebilir</p>
                  )}
                </div>
              </div>
              ...
              <div className="card-dark p-6 flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="font-display font-bold text-lg mb-1">E-posta</h3>
                  {s.email ? (
              ...
              <div className="card-dark p-6 flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="font-display font-bold text-lg mb-1">Çalışma Saatleri</h3>
                  {s.hours_weekday || s.hours_weekend ? (
                    <div className="text-white/60 space-y-1" data-testid="contact-hours">
                      {s.hours_weekday && <p>Hafta içi: {s.hours_weekday}</p>}
                      {s.hours_weekend && <p>Hafta sonu: {s.hours_weekend}</p>}
                    </div>
                  ) : (
                    <p className="text-white/30 italic text-sm">Admin panelinden eklenebilir</p>
                  )}
